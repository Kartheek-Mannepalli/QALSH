	...
	int b_length)						// block length
{
	...
	if ((fp_ = fopen(name, "rb+")) != 0) {
		...
		if (block_length_ < BFHEAD_LENGTH) {
			...
			// -----------------------------------------------------------------
			printf("BlockFile::BlockFile couldnot open file.\n");
			exit(EXIT_FAILURE);
		}
...
BlockFile::~BlockFile()				// destructor
{
	if (file_name_) {				// release space of <file_name_>
		delete[] file_name_; file_name_ = nullptr;
	}
	...
	int index)							// pos of the block
{
    ...
    auto start_index_io_time = std::chrono::steady_clock::now();
    ...
    auto end_index_io_time = std::chrono::steady_clock::now();
    INDEX_IO_TIME += std::chrono::duration <double, std::milli>
            (end_index_io_time - start_index_io_time).count();

    if (VR_FLAG)
        VR_IO_TIME += std::chrono::duration <double, std::milli>
                (end_index_io_time - start_index_io_time).count(); // VR Index IO time (omid)

    if (index + 1 > num_blocks_) {	// <fp_> reaches the end of file
        start_index_io_time = std::chrono::steady_clock::now();

        fseek(fp_, 0, SEEK_SET);

        end_index_io_time = std::chrono::steady_clock::now();
        INDEX_IO_TIME += std::chrono::duration <double, std::milli>
                (end_index_io_time - start_index_io_time).count();
