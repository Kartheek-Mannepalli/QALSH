	...
	BTree* btree)						// b-tree of this node
{
	...
	if (capacity_ < 50) {			// ensure at least 50 entries
		printf("capacity = %d\n", capacity_);
		printf("BIndexNode::init() capacity too small.\n");
		exit(EXIT_FAILURE);
	}
	...
	int block)							// addr of disk for this node
{
	...
	if (capacity_ < 50) {			// at least 50 entries
		printf("capacity = %d\n", capacity_);
		printf("BIndexNode::init_restore capacity too small.\n");
		exit(EXIT_FAILURE);
	}
	...
	int index)							// input index
{
	if (index < 0 || index >= num_entries_) {
	    printf("BIndexNode::get_key out of range.\n");
		exit(EXIT_FAILURE);
	}
	return key_[index];
}
...
{
	if (index < 0 || index >= num_entries_) {
	    printf("BIndexNode::get_son out of range.\n");
		exit(EXIT_FAILURE);
	}
	return son_[index];
}
	...
	int son)							// input son
{
	if (num_entries_ >= capacity_) {
	    printf("BIndexNode::add_new_child overflow\n");
		exit(EXIT_FAILURE);
	}
	...
	BTree* btree)						// b-tree of this node
{
	...
	if (capacity_ < 100) {			// at least 100 entries
		printf("capacity = %d\n", capacity_);
		printf("BLeafNode::init capacity too small.\n");
		exit(EXIT_FAILURE);
	}
	...
	int block)							// addr of disk for this node
{
	...
	if (capacity_ < 100) {			// at least 100 entries
		printf("capacity = %d\n", capacity_);
		printf("BLeafNode::init_store capacity too small.\n");
		exit(EXIT_FAILURE);
	}
	id_ = new int[capacity_];		// init <id>
	for (int i = 0; i < capacity_; i++) {
	...
	int index)							// input <index>
{
	if (index < 0 || index >= num_keys_) {
	    printf("BLeafNode::get_key out of range.\n");
	    exit(EXIT_FAILURE);
	}
	return key_[index];
}
...
{
	if (index < 0 || index >= num_entries_) {
	    printf("BLeafNode::get_entry_id out of range.\n");
		exit(EXIT_FAILURE);
	}
	return id_[index];
}
...
{
	if (num_entries_ >= capacity_) {
	    printf("BLeafNode::add_new_child entry overflow\n");
	    exit(EXIT_FAILURE);
	}
	...
	if ((num_entries_ * SIZEINT) % INDEX_SIZE_LEAF_NODE == 0) {
		if (num_keys_ >= capacity_keys_) {
		    printf("BLeafNode::add_new_child key overflow\n");
			exit(EXIT_FAILURE);
		}
