...
{
	if (root_ptr_ != nullptr) {		// release <root_ptr_>
		delete root_ptr_; root_ptr_ = nullptr;
	}
	...
	if (file_ != nullptr) {			// release <file_>
	...
	int b_length)						// block length
{
	...
	if (fp) {						// check whether the file exist
		...
		if (c != 'y' && c != 'Y') {	// if not remove existing file
			exit(EXIT_FAILURE);		// program will be stopped.
		}
	...
	int n)								// number of entries
{
	...
	for (int i = 0; i < n; i++) {
		...
		if (!leaf_act_nd) {
			...
			if (first_node) {
				...
				delete leaf_prev_nd; leaf_prev_nd = nullptr;
			}
	...
	if (leaf_prev_nd != nullptr) {		// release the space
		delete leaf_prev_nd; leaf_prev_nd = nullptr;
	}
	if (leaf_act_nd != nullptr) {
		delete leaf_act_nd; leaf_act_nd = nullptr;
	}
	...
	while (last_end_block > last_start_block) {
		...
		for (int i = last_start_block; i <= last_end_block; i++) {
			...
			if (current_level == 1) {
				...
				delete leaf_child; leaf_child = nullptr;
			}
			else {
				...
				delete index_child; index_child = nullptr;
			}
			...
			if (!index_act_nd) {
				...
				if (first_node) {
					...
					delete index_prev_nd; index_prev_nd = nullptr;
				}
		...
		if (index_act_nd != nullptr) {
			delete index_act_nd; index_act_nd = nullptr;
		}
									// update info
		last_start_block = start_block;
