	...
	float** points)						// data or queries (return)
{
	...
	if (!feof(fp) && i == n) {		// check the size of set
		...
		printf("Set the size of dataset to be %d. And try again\n", i);
        exit(EXIT_FAILURE);
	}
	...
	char* output_path)					// output path
{
		...
		if (data_path[i] == '/') {
			...
			if (ret != 0) {			// create the directory
				...
				if (ret != 0) {
					...
					printf("write_data_new_form error\n");
					exit(EXIT_FAILURE);
				}
			}
			data_path[i + 1] = ch;
	...
	for (int i = 0; i < total_file; i++) {
		...
		if (write_buffer_to_page(B, fname, buffer) == 1) {
		    printf("write_data_new_form error to write a page\n");
		    exit(EXIT_FAILURE);
		}
	}
	...
	if (data_path != nullptr || fname != nullptr) {
...
int write_buffer_to_page(			// write buffer to one page
	...
	char* buffer)						// buffer to store data
{
	if (fname == nullptr || buffer == nullptr) return 1;
	...
	char* output_path)					// output path
{
	...
	if (read_buffer_from_page(B, fname, buffer) == 1) {
	    printf("read_data() error to read a page\n");
	    exit(EXIT_FAILURE);
	}
